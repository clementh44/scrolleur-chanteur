...
<div class="undo-list">
    <div v-for="(deletedElement, index) in deletedElements" :key="index" class="toast fade show" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="progress d-flex" style="height: 4px">
            <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" :aria-valuenow="deletedElement.width" :style="{width: deletedElement.width + '%', transition: 'none'}" aria-valuemin="0" aria-valuemax="100"></div>
        </div>
        ...
        <div class="toast-body">
            <button @click="restore(deletedElement)" type="button" class="btn btn-primary btn-sm float-right">Restaurer</button>"{{ deletedElement.title }}"
        </div>
    </div>
</div>
</template>
...
export default {
    ...
    methods: {
        addElement(element, title) {
            ...
            deletedElement.timeout = setTimeout(() => {this.deleteElement(deletedElement)}, 5000)
            deletedElement.interval = setInterval(() => {deletedElement.width = deletedElement.width-0.20}, 10)
